...
class Router
	def call(env)
		...
		#return [200, {'Content-Type' => 'text/html', 'charset' => 'utf-8'}, [ 'Rabotaet SHURA - ne meshat' ]] unless start.env.client_ip == '81.20.204.174' 
		if request_path.match(%r{^/info}) then return start.info end
		...
		if CGI.unescape(request_path).match(%r{^/feed/search/([А-Яа-яA-Za-z0-9]{2,32})$}) then return start.mind('mind_random', { :h => "##{$1}"}) end

		if request_path.match(%r{^/mind$}) then return start.mind end
		if request_path.match(%r{^/pool$}) then return start.pool end
		
		if request_path.match(%r{^/feed/my$})  then return start.notice_mind_added         end
		if request_path.match(%r{^/feed/all$}) then return start.mind('mind_random')       end
		if request_path.match(%r{^/feed/hot$}) then return start.minding('minds_read_top') end
		if request_path.match(%r{^/minds$})    then return start.field_users               end

		if request_path.match(%r{^/minds/([a-z0-9]{4,32})$}) then return start.field_users($1)     end
		if request_path.match(%r{^/poll/([0-9a-fA-F]{24})})  then return start.mind('mind_one',$1) end
		...
		if request_path.match(%r{^/tags/top$}) then return start.tags('top') end
